...
VALID_CHARACTERS = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z", "ä", "ö", "ü", "ß"]
amira_index: int = 0
bela_index: int = 1
...
def main() -> None:
    while True:
        user_input: str = input("Bitte gebe einen Pfad zu einer .txt Datei an! ")
        text: str = get_text_to_proccess(user_input)
        text = format_text_correctly(text)
        #while True:
        #    amira_character = text[amira_index]
        #    bela_character = text[bela_index]
